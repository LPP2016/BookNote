	...
	\section{基本概念}
		\subsection{仿射集（2019年7月2日）}
			...
			\begin{definition}[仿射包]
				给定$S \subset \textbf{R}^{n}$（$S$是一个集合），存在唯一一个包含$S$的最小的仿射集（例如，满足$S \subset M$的所有仿射集的交集）。这个仿射集被称为$S$的仿射包（afine hull），记作aff$S$。aff$S$可以由所有形式为$\lambda_{1}\textbf{x}_{1}+...+\lambda_{m}\textbf{x}_{m}$向量组成，其中$\textbf{x}_{i} \in S$且$\lambda_{1}+...+\lambda_{m}=1$。
			\end{definition}
